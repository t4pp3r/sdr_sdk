...
bool aim::calculate_hitchance(int& final_hitchance)
{
	...
	for (auto i = 0u; i < 255; i++)  // NOLINT(modernize-loop-convert)
	{
		...
		// an?er? oolnr.
		const auto round_acc = [](const float accuracy) { return std::roundf(accuracy * 1000.f) / 1000.f; };
		const auto sniper = g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_AWP || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_G3SG1 || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SCAR20 || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SSG08;
		const auto crouched = g_ctx.local()->m_fFlags() & FL_DUCKING;
		//const auto weapon_inaccuracy = weapon_inaccuracy;
		const auto weapon_spread = g_ctx.globals.weapon->get_spread();

		// neune n?cnrnu ocnrrin, lnec eu lai ooe nalerle aieurl?.
		if (crouched) {
			if (round_acc(weapon_inaccuracy) == round_acc((sniper && g_ctx.globals.weapon->m_zoomLevel()) ? info->flInaccuracyCrouchAlt : info->flInaccuracyCrouch))
				return true;
		}
		else if (round_acc(weapon_inaccuracy) == round_acc((sniper && g_ctx.globals.weapon->m_zoomLevel()) ? info->flInaccuracyStandAlt : info->flInaccuracyStand))
			return true;
		...
		// riau nndle?nu a rcdro ni neronie.
...
float aim::GetPointScale(float flHitboxRadius, Vector* vecPoint, Vector* vecEyePos, int iHitbox) {
	...
	if (g_cfg.ragebot.enable) {
		...
		v7 = vecPoint->z - vecEyePos->z;
		v6 = g_flSpread + g_flInaccurarcy;

		pDist = veсPoint->DistTo(*vecEyePos) / std::sin(math::Deg2Rad(90.f - math::Rad2Deg(v7)));
		RadSpread.x = ((90.0 - (v6 * 57.29578) * 0.017453292));
		RadSpread.x = sinf(RadSpread.x);
		v11 = pDist / RadSpread.x;
		dCone = v6;
		ScaleState = v11;
		dCone = sinf(float(v6));
...
void aim::doubletapsafe(CUserCmd* cmd)
{
	if (misc::get().double_tap_enabled)
		return;

	auto hitboxes = (HITBOX_CHEST, HITBOX_LEFT_FOOT, HITBOX_RIGHT_FOOT, HITBOX_STOMACH);


	if (hitboxes)
		BAIM_DOUBLE_TAP; true;

}

void aim::add_matrix(player_t* player, matrix3x4_t* bones)
{
	auto& hit = g_ctx.m_Hitmatrix.emplace_back();
...
int aim::hitchance(const Vector& aim_angle)
{
	...
	float inaccuracy1, spread1;
	float hitchance = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount;
	Vector	 m_shoot_pos;
	Vector     start{ m_shoot_pos }, end, fwd, right1, up1, dir, wep_spread;

	if (m_shoot_next_tick)
		HITCHANCE_MAX += 25;

	size_t     total_hits{ }, needed_hits{ (size_t)std::ceil((hitchance * SEED_MAX) / HITCHANCE_MAX) };
	math::angle_vectors(aim_angle, &fwd, &right1, &up1);

	inaccuracy1 = g_ctx.globals.inaccuracy;
	spread1 = g_ctx.globals.spread;

	for (int i{ }; i <= SEED_MAX; ++i) {

		// get spread direction.
		dir = (fwd + (right1 * wep_spread.x) + (up1 * wep_spread.y));
