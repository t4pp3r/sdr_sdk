...
void otheresp::indicators()
{
	...
	if (g_cfg.esp.indicators[INDICATOR_LD] && g_cfg.bind.low_delta_fix && g_cfg.bind.low_delta_fix_key.key > KEY_NONE && g_cfg.bind.low_delta_fix_key.key < KEY_MAX && misc::get().low_delta_fix_key)
		m_indicators.push_back(m_indicator("LOWDELTA FIX", !g_ctx.local()->m_bGunGameImmunity() && !(g_ctx.local()->m_fFlags() & FL_FROZEN) && !antiaim::get().freeze_check && misc::get().low_delta_fix_enabled ? Color(130, 170, 20) : Color(130, 20, 20)));
}
...
void otheresp::automatic_peek_indicator()
{
	...
	if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(18) || alpha)
	{
		...
		render::get().Draw3DFilledCircle(position, 15.0f, g_cfg.esp.molotov_timer ? Color(183, 206, 232, (int)(alpha * 55.0f)) : Color(183, 206, 232, (int)(alpha * 55.0f)));
		Vector screen;
