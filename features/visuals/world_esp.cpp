...
void worldesp::draw_hit_matrix()
{
	if (!g_ctx.available())
		g_ctx.m_Hitmatrix.clear();

	if (g_ctx.m_Hitmatrix.empty())
		return;
	...
	auto it = g_ctx.m_Hitmatrix.begin();
	while (it != g_ctx.m_Hitmatrix.end()) {
...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	static auto inferno_flame_lifetime = m_cvar()->FindVar("inferno_flame_lifetime");
	...
	auto factor = (spawn_time + inferno_lifetime) - m_globals()->m_curtime;
	...
	if (factor >= 0.f) {
		...
		render::get().line(screen_origin.x - size.x * 0.5f, screen_origin.y - size.y * 0.5f, size.x, size.y, Color(255, 255, 255, 80));
	}
}

void worldesp::molotov_troll(entity_t* entity)
{
	if (!g_cfg.esp.molotov_troll)
		return;
	...
	render::get().rect_filled(screen_origin.x - size.x * 0.5f, screen_origin.y - size.y * 0.5f - 1.0f, size.x, size.y, Color(37, 37, 37, g_cfg.esp.molotov_troll_color.a()));
	render::get().rect_filled(screen_origin.x - size.x * 0.5f + 2.0f, screen_origin.y - size.y * 0.5f, (size.x - 4.0f) * factor, size.y - 2.0f, g_cfg.esp.molotov_troll_color);

	render::get().rect(screen_origin.x - size.x * 0.5f, screen_origin.y - size.y * 0.5f, size.x, size.y, Color(7, 7, 7, g_cfg.esp.molotov_troll_color.a()));
	render::get().text(fonts[ESP], screen_origin.x, screen_origin.y - size.y * 0.5f + 12.0f, g_cfg.esp.molotov_troll_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "FIRE");
	render::get().text(fonts[GRENADES], screen_origin.x + 1.0f, screen_origin.y - size.y * 0.5f - 9.0f, g_cfg.esp.molotov_troll_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, "l");
}
...
void worldesp::smoke_timer(entity_t* entity)
{
	...
	render::get().Draw3DFilledCircle(origin, 110.0f, g_cfg.esp.smoke_timer_color);
}
