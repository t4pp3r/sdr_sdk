...
#include "..\misc\misc.h"
#include "..\misc\logs.h"
#include "..\somefixes\setup_bones.h"
...
void lagcompensation::update_player_animations(player_t* e)
{
...
#endif
	{
		...
		if (g_cfg.player_list.low_delta[e->EntIndex()])
		{
			switch (record->side)
			{
				auto lowdeltafix1 = math::random_float(17.f, 28.f);

				auto animstate1 = e->get_animation_state();

				switch (g_ctx.globals.missed_shots[e->EntIndex()] % 1)
				{
				case 0:
					animstate1->m_flGoalFeetYaw = lowdeltafix1;
					break;
				default:
					break;
				}
