...
#include "..\ragebot\prediction_system.h"
#include "logs.h"

#if RELEASE
#if BETA
#define VERSION crypt_str("sdr [beta] | ")
#else
#define VERSION crypt_str("sdr | ")
#endif
#else
#define VERSION crypt_str("sdr [alpha] | ")
#endif

#define SUB crypt_str("You have subscription!")
...
{
	return mouse_pos.x > position.x && mouse_pos.y > position.y && mouse_pos.x < position.x + size.x && mouse_pos.y < position.y + size.y;
}
...
void misc::watermark()
{
	...
	auto watermark = VERSION + comp_name() /*g_ctx.username*/ + (" | ") + g_ctx.globals.time + (" | ") + SUB;
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = VERSION + comp_name() /*g_ctx.username*/ + (" | ") + server + (" | delay: ") + std::to_string(g_ctx.globals.ping) + (" ms | ") + tickrate + (" tick | ") + g_ctx.globals.time;
		}
	}

	/*auto box_width = render::get().text_width(fonts[NAME], watermark.c_str()) + 10;

	render::get().gradient(width - 10 - box_width, 10, box_width / 2, 1, Color(25, 25, 250), Color(25, 25, 250), GRADIENT_HORIZONTAL);
	render::get().gradient(width - 10 - box_width + (box_width / 2), 10, box_width / 2, 1, Color(25, 25, 250), Color(25, 25, 250), GRADIENT_HORIZONTAL);
	...
	render::get().rect_filled(width - 10 - box_width, 10, box_width / 2, 1, Color(25, 25, 250));
	render::get().rect_filled(width - 10 - box_width + (box_width / 2), 10, box_width / 2, 1, Color(25, 25, 250));
...
void misc::keybinds()
{
	...
	if (g_cfg.ragebot.double_tap_key.key > KEY_NONE && g_cfg.ragebot.double_tap_key.key < KEY_MAX && misc::get().double_tap_key)
	{
		render::get().text(fonts[NAME], 98, 360 + 12 + 13 * offset, Color(255, 255, 255), true, "> doubletap");
		offset = offset + 1;
	}

	if (g_cfg.antiaim.hide_shots_key.key > KEY_NONE && g_cfg.antiaim.hide_shots_key.key < KEY_MAX && misc::get().hide_shots_key)
	{
	...
	{
		render::get().text(fonts[NAME], 98, 360 + 12 + 13 * offset, Color(255, 255, 255), true, "> fake duck");
		offset = offset + 1;
	}

	if (g_cfg.esp.indicators[INDICATOR_DAMAGE] && g_ctx.globals.current_weapon != -1 && key_binds::get().get_key_bind_state(4 + g_ctx.globals.current_weapon))
	{
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("sdr - stay with us or lose the game."),
		crypt_str("Get good. Get sdr."),
		crypt_str("sdr - just the best."),
		crypt_str("Go to another level with sdr."),
	};
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	{
		auto recharge_time = g_ctx.globals.weapon->can_double_tap() ? TIME_TO_TICKS(0.75f) : TIME_TO_TICKS(0.75f); // 1.5
	...
	auto tickbase_shift = min(g_cfg.ragebot.double_tap_shift_value, max_tickbase_shift);
	...
	if (!g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && g_ctx.send_packet && (m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2 && g_ctx.globals.weapon->is_knife())) //-V648
	{
		...
		{
			g_ctx.globals.double_tap_aim = true; // true
			g_ctx.globals.double_tap_aim_check = true; // true
		}

		recharge_double_tap = true; // true
		double_tap_enabled = false;
		double_tap_key = false;
...
void misc::hide_shots(CUserCmd* m_pcmd, bool should_work)
{
	...
	g_ctx.globals.next_tickbase_shift = m_gamerules()->m_bIsValveDS() ? 6 : 9; // 6 : 9
