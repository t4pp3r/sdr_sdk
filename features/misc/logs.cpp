...
void eventlogs::paint_traverse()
{
	...
	for (unsigned int i = 0; i < logs.size(); i++) {
		...
		float logsBG[4] = { 0.11f, 0.14f, 0.24f, 1.f };
...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ NEVERMISS V2 BETA ] ")); //-V807
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ NEVERMISS V2 ] "));
#endif
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ NEVEMISS V2 ALPHA ] ")); //-V807
#endif
...
#if RELEASE
#if BETA
		auto log = crypt_str("[ \x0CANEVERMISS V2 BETA \x01] ") + text;
		chat->chat_print(log.c_str());
#else
		auto log = crypt_str("[ \x0CNEVERMISS V2 \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
#else
		auto log = crypt_str("[ \x0CNEVERMISS V2 ALPHA \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
	}
}